...
#include "commands/DriveHeadingTimed.h"

DriveHeadingTimed::DriveHeadingTimed(SwerveDrive* swerveDrive, double cmdSpeed, uint16_t timeout)
: m_swerveDrive(swerveDrive), m_targetSpeed(cmdSpeed), m_timeout(timeout)
{
    SetName("DriveHeadingTimed");
    m_timer = 0;
}

// Called just before this Command runs the first time
void DriveHeadingTimed::Initialize() 
{
    m_timer = m_timeout;
...
void DriveHeadingTimed::Execute() 
{
    if(m_timer) m_timer--;
    m_swerveDrive->HeadingSwerve(m_targetSpeed);
...
bool DriveHeadingTimed::IsFinished() 
{
    return(m_timer == 0);
...
void DriveHeadingTimed::End(bool interrupted) 
{
    m_timer = 0;
    m_swerveDrive->initHeading();
...
bool DriveHeadingTimed::RunsWhenDisabled() const 
{
    return false;
